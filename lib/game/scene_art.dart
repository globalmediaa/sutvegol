...
class SceneArt {
  ...
  static Future<ui.Image> splashSky() {
    ...
    c.drawRect(
      ...
      Paint()
        ..shader = ui.Gradient.linear(
          Offset.zero,
          const Offset(0, h),
          const [Color(0xFF060A1C), Color(0xFF12173A), Color(0xFF2B1E5C)],
          const [0, 0.55, 1],
        ),
    );
    final rng = Random(11);
    for (var i = 0; i < 160; i++) {
      ...
      c.drawCircle(
        Offset(rng.nextDouble() * kWorldW, y),
        1.5 + rng.nextDouble() * 2.5,
        Paint()..color = Color.fromRGBO(255, 255, 255, a.clamp(0, 1)),
      );
    }
    ...
    c.drawRect(
      ...
      Paint()
        ..shader =
            ui.Gradient.radial(const Offset(kWorldW * 0.3, h), kWorldW * 1.1, [
  ...
  static void paintBall(Canvas c, double r, {bool king = false}) {
    c.save();
    c.clipPath(
      Path()..addOval(Rect.fromCircle(center: Offset.zero, radius: r - 0.5)),
    );
    final base = king
        ? const [Color(0xFFFFF3B8), Color(0xFFF7C23A), Color(0xFFC87A08)]
        : const [Color(0xFFFFFFFF), Color(0xFFEDF0F6), Color(0xFFB9C1D3)];
    c.drawCircle(
      Offset.zero,
      r,
      Paint()
    ...
    final panel = Paint()
      ..color = king ? const Color(0xFF8A4A00) : const Color(0xFF1E2230);
    final seam = Paint()
  ...
  static void _drawTruncatedIcosahedron(
    Canvas c,
    double r,
    Paint panel,
    Paint seam,
    ...
    for (final ctr in centers) {
      ...
      final near = List<int>.generate(verts.length, (i) => i)
        ..sort((a, b) => dot(verts[b], ctr).compareTo(dot(verts[a], ctr)));
      ...
      five.sort(
        (a, b) => atan2(
          dot(verts[a], v),
          dot(verts[a], u),
        ).compareTo(atan2(dot(verts[b], v), dot(verts[b], u))),
      );
      final path = Path();
      for (var i = 0; i < 5; i++) {
    ...
    c.drawPath(
      ...
      Paint()
        ..shader = ui.Gradient.linear(
          ...
          Offset(0, h / 2),
          dead
              ? const [Color(0xFF3A4157), Color(0xFF232A3E)]
              : [color, Color.lerp(color, const Color(0xFF7A0020), 0.45)!],
        ),
    );
    if (!dead) {
      c.drawOval(
        Rect.fromCenter(
          center: Offset(-w * 0.2, -h * 0.18),
          width: w * 0.22,
          height: h * 0.16,
        ),
  ...
  static void paintTarget(Canvas c, double r, Color accent, double alpha) {
    ...
    c.drawCircle(
      ...
      r * 0.86,
      Paint()
        ..color = const Color(0xFF0B1226).withValues(alpha: 0.55 * alpha)
        ..style = PaintingStyle.stroke
        ..strokeWidth = r * 0.30,
    );
    c.drawCircle(
  ...
  static void paintKeeper(Canvas c, double w, double h, double alpha) {
    final fill = Paint()
      ..color = const Color(0xFF1C2230).withValues(alpha: alpha);
    final leg = Paint()
    ...
    // Aşınma lekeleri.
    final spot = Paint()
      ..color = const Color(0xFF0B0E16).withValues(alpha: 0.6 * alpha);
  ...
  static void _sky(Canvas c, Stage stage, Random rng) {
    ...
    if (stage == Stage.street) {
      ...
      c.drawCircle(
        const Offset(1010, 300),
        92,
        Paint()..color = const Color(0xFFFFF6C8),
      );
      _clouds(
        c,
        rng,
        ...
        c.drawCircle(
          ...
          Paint()
            ..color = Color.fromRGBO(
              ...
              255,
              255,
              (0.3 + rng.nextDouble() * 0.6) * (1 - y / 1000),
            ),
        );
      }
      if (stage == Stage.cage) {
        c.drawCircle(
          const Offset(1060, 360),
          140,
          Paint()
            ..color = const Color(0x33CFE6FF)
            ..maskFilter = const MaskFilter.blur(BlurStyle.normal, 60),
        );
        c.drawCircle(
          const Offset(1060, 360),
          68,
          Paint()..color = const Color(0xFFF2F6FF),
        );
        c.drawCircle(
          const Offset(1085, 345),
          58,
          Paint()..color = const Color(0xFF0A1330),
        );
      } else {
        _clouds(
          c,
  ...
  static void _clouds(
    ...
    for (var i = 0; i < n; i++) {
      ...
      c.drawRRect(
        RRect.fromRectAndRadius(
          Rect.fromCenter(
            center: Offset(x - w * 0.08, y - h * 0.22),
            width: w * 0.6,
            height: h * 0.45,
          ),
          Radius.circular(h / 2),
  ...
  static void _streetBackdrop(Canvas c, ViewGeom g, Random rng) {
    ...
    while (x < kWorldW + 40) {
      ...
      if (rng.nextDouble() < 0.4) {
        c.drawRect(Rect.fromLTWH(x + w * 0.3, top - 60, 8, 60), far);
      }
      ...
      for (var wy = top + 30; wy < wallTop - 10; wy += 44) {
        for (var wx = x + 14; wx < x + w - 20; wx += 30) {
          if (rng.nextDouble() < 0.35) {
            c.drawRect(Rect.fromLTWH(wx, wy, 14, 20), win);
          }
        }
      }
    ...
    for (final b in [(0.0, 340.0, 0xFF8C5A47), (960.0, 1320.0, 0xFF6E4A3E)]) {
      ...
      c.drawRect(
        Rect.fromLTRB(rect.left, rect.top, rect.right, rect.top + 26),
        Paint()..color = const Color(0xFF4A2E2A),
      );
      for (var wy = rect.top + 60; wy < rect.bottom - 40; wy += 96) {
        for (var wx = rect.left + 40; wx < rect.right - 60; wx += 90) {
          ...
          c.drawRRect(
            RRect.fromRectAndRadius(
              Rect.fromLTWH(wx, wy, 52, 64),
              const Radius.circular(4),
            ),
            Paint()
              ..color = lit ? const Color(0xFFFFD27A) : const Color(0xFF2F2233),
          );
          c.drawRect(
            Rect.fromLTWH(wx - 6, wy + 64, 64, 8),
            Paint()..color = const Color(0xFF4A2E2A),
          );
        }
      }
    ...
    c.drawRect(
      ...
      Paint()
        ..shader = ui.Gradient.linear(
          Offset(0, wallTop),
          Offset(0, wallBottom),
          const [Color(0xFFB0644A), Color(0xFF8E4B36)],
        ),
    );
    final mortar = Paint()
    ...
    for (var y = wallTop + 46; y < wallBottom; y += 46) {
      ...
      for (var bx = -40 + off; bx < kWorldW + 40; bx += 124) {
        ...
        if (rng.nextDouble() < 0.12) {
          c.drawRect(
            Rect.fromLTWH(bx + 4, y - 42, 116, 38),
            Paint()
              ..color = Color.fromRGBO(
                255,
                230,
    ...
    c.drawRect(
      Rect.fromLTRB(-40, wallTop - 18, kWorldW + 40, wallTop + 8),
      Paint()..color = const Color(0xFFC99B7A),
    );
    // Grafiti: yumuşak lekeler ve kalın fırça izleri.
    for (final s in [
      (160.0, wallTop + 260, 0xFF35D5F2),
      (1130.0, wallTop + 330, 0xFFFF7A1A),
      (260.0, wallTop + 420, 0xFFFFC53D),
    ]) {
      c.drawOval(
        Rect.fromCenter(center: Offset(s.$1, s.$2), width: 240, height: 130),
        Paint()
          ..color = Color(s.$3).withValues(alpha: 0.55)
    ...
    const lx = 1200.0;
    c.drawRect(
      Rect.fromLTWH(lx - 9, wallTop - 560, 18, wallBottom - wallTop + 560),
      Paint()..color = const Color(0xFF2A2E38),
    );
    c.drawRRect(
      RRect.fromRectAndRadius(
        Rect.fromLTWH(lx - 70, wallTop - 600, 140, 44),
    ...
    c.drawRRect(
      RRect.fromRectAndRadius(
        Rect.fromLTWH(lx - 52, wallTop - 566, 104, 16),
    ...
    c.drawPath(
      ...
      Paint()
        ..shader = ui.Gradient.linear(
          Offset(0, wallTop - 560),
          Offset(0, wallBottom),
          const [Color(0x66FFE2A0), Color(0x00FFE2A0)],
        ),
    );
    // Çöp konteyneri (sol).
    c.drawRRect(
      RRect.fromRectAndRadius(
        Rect.fromLTWH(40, wallBottom - 190, 250, 190),
    ...
    c.drawRRect(
      RRect.fromRectAndRadius(
        Rect.fromLTWH(30, wallBottom - 214, 270, 40),
        const Radius.circular(10),
      ),
      Paint()..color = const Color(0xFF3B865C),
    );
    c.drawRect(
      Rect.fromLTWH(60, wallBottom - 140, 210, 8),
      Paint()..color = const Color(0x55000000),
    );
    // Duvar dibi gölgesi.
    c.drawRect(
      Rect.fromLTRB(-40, wallBottom - 90, kWorldW + 40, wallBottom),
      Paint()
        ..shader = ui.Gradient.linear(
          Offset(0, wallBottom - 90),
          Offset(0, wallBottom),
  ...
  static void _beachBackdrop(Canvas c, ViewGeom g, Random rng) {
    ...
    c.drawRect(
      ...
      Paint()
        ..shader = ui.Gradient.linear(
          Offset(0, horizon),
          Offset(0, shore),
          const [Color(0xFF1B6FD0), Color(0xFF2AA8DD), Color(0xFF4FD0E6)],
          const [0, 0.55, 1],
        ),
    );
    // Ufukta yelkenli.
    ...
    c.drawPath(
      Path()
        ..moveTo(300, horizon + 6)
        ..lineTo(300, horizon - 70)
        ..lineTo(350, horizon + 6)
        ..close(),
      sail,
    );
    c.drawPath(
      Path()
        ..moveTo(280, horizon + 8)
        ..lineTo(372, horizon + 8)
        ..lineTo(360, horizon + 22)
        ..lineTo(292, horizon + 22)
        ..close(),
    ...
    for (var i = 0; i < 80; i++) {
      ...
      c.drawPath(
        Path()
          ..moveTo(x - w / 2, y)
          ..quadraticBezierTo(x, y - 6 - 10 * t, x + w / 2, y),
        Paint()
    ...
    c.rotate(-0.12);
    c.drawRect(
      const Rect.fromLTWH(-6, -330, 12, 330),
      Paint()..color = const Color(0xFFE9E3D5),
    );
    for (var i = 0; i < 8; i++) {
      c.drawArc(
        Rect.fromCircle(center: const Offset(0, -320), radius: 160),
        pi + i * pi / 8,
        pi / 8,
        true,
        Paint()
      ...
      c.drawPath(
        ...
        Paint()
          ..color = i.isEven
              ...
              : const Color(0xFF3BB662),
      );
    }
    c.drawCircle(
      top + const Offset(0, 8),
      16,
      Paint()..color = const Color(0xFF7A4B26),
    );
  }
  ...
  static void _cageBackdrop(Canvas c, ViewGeom g, Random rng) {
    ...
    while (x < kWorldW + 40) {
      ...
      for (var wy = top + 24; wy < fenceTop; wy += 36) {
        for (var wx = x + 12; wx < x + w - 14; wx += 26) {
          if (rng.nextDouble() < 0.3) {
            c.drawRect(
              Rect.fromLTWH(wx, wy, 10, 14),
              Paint()
                ..color = rng.nextBool()
                    ? const Color(0xAAFFE08A)
                    : const Color(0xAA8FD3FF),
            );
          }
        }
      }
    ...
    c.drawRect(
      Rect.fromLTRB(-40, fenceTop, kWorldW + 40, fenceBottom),
      Paint()..color = const Color(0xFF0E1A33),
    );
    for (final lx in [150.0, 1170.0]) {
      c.drawRect(
        Rect.fromLTWH(
          ...
          fenceTop - 520,
          24,
          fenceBottom - fenceTop + 520,
        ),
        Paint()..color = const Color(0xFF3A4150),
      );
      c.drawRRect(
        RRect.fromRectAndRadius(
          Rect.fromLTWH(lx - 110, fenceTop - 600, 220, 90),
          const Radius.circular(8),
        ),
        Paint()..color = const Color(0xFF2A303C),
      );
      for (var i = 0; i < 4; i++) {
        c.drawRRect(
          RRect.fromRectAndRadius(
            Rect.fromLTWH(lx - 98 + i * 52, fenceTop - 588, 42, 66),
            const Radius.circular(6),
          ),
          Paint()..color = const Color(0xFFF4F9FF),
        );
      }
      c.drawCircle(
        Offset(lx, fenceTop - 555),
        200,
        Paint()
          ..color = const Color(0x44CFE6FF)
          ..maskFilter = const MaskFilter.blur(BlurStyle.normal, 80),
      );
      ...
      c.drawPath(
        ...
        Paint()
          ..shader = ui.Gradient.linear(
            Offset(0, fenceTop - 520),
            Offset(0, fenceBottom + 300),
            const [Color(0x4CCFE6FF), Color(0x00CFE6FF)],
          ),
      );
    }
    ...
    for (var d = -kWorldH; d < kWorldW + kWorldH; d += sp) {
      c.drawLine(
        Offset(d, fenceTop),
        Offset(d + (fenceBottom - fenceTop), fenceBottom),
        mesh,
      );
      c.drawLine(
        Offset(d, fenceBottom),
        Offset(d + (fenceBottom - fenceTop), fenceTop),
        mesh,
      );
    }
    c.restore();
    for (var px = 60.0; px < kWorldW; px += 240) {
      c.drawRRect(
        RRect.fromRectAndRadius(
          Rect.fromLTWH(px - 8, fenceTop - 10, 16, fenceBottom - fenceTop + 10),
          const Radius.circular(4),
        ),
        Paint()..color = const Color(0xFF1E5F3A),
      );
    }
    c.drawRect(
      Rect.fromLTRB(-40, fenceTop - 14, kWorldW + 40, fenceTop),
      Paint()..color = const Color(0xFF2A7A4C),
    );
    c.drawRect(
      Rect.fromLTRB(-40, fenceBottom - 80, kWorldW + 40, fenceBottom),
      Paint()
        ..shader = ui.Gradient.linear(
          Offset(0, fenceBottom - 80),
          Offset(0, fenceBottom),
  ...
  static void _stadiumBackdrop(Canvas c, ViewGeom g, Random rng) {
    ...
    for (final lx in [110.0, 1210.0]) {
      c.drawRect(
        Rect.fromLTWH(lx - 10, standTop - 380, 20, 400),
        Paint()..color = const Color(0xFF2A3044),
      );
      c.drawRRect(
        RRect.fromRectAndRadius(
          Rect.fromLTWH(lx - 90, standTop - 470, 180, 100),
          const Radius.circular(8),
        ),
        Paint()..color = const Color(0xFF2A3044),
      );
      for (var r = 0; r < 2; r++) {
        for (var i = 0; i < 4; i++) {
          c.drawRRect(
            RRect.fromRectAndRadius(
              Rect.fromLTWH(lx - 82 + i * 42, standTop - 462 + r * 46, 36, 40),
      ...
      c.drawCircle(
        Offset(lx, standTop - 420),
        260,
        Paint()
          ..color = const Color(0x55FFF4CC)
          ..maskFilter = const MaskFilter.blur(BlurStyle.normal, 100),
      );
    }
    // Çatı.
    c.drawRect(
      Rect.fromLTRB(-40, standTop - 90, kWorldW + 40, standTop),
      Paint()
        ..shader = ui.Gradient.linear(
          Offset(0, standTop - 90),
          Offset(0, standTop),
          const [Color(0xFF3A4260), Color(0xFF1B2032)],
        ),
    );
    for (var x = 20.0; x < kWorldW; x += 120) {
      c.drawRect(
        Rect.fromLTWH(x, standTop - 40, 60, 14),
    ...
    for (final t in tiers) {
      ...
      for (var y = t.top + 10; y < t.bottom - 6; y += 15) {
        for (var x = t.left + 6; x < t.right; x += 15) {
          ...
          c.drawCircle(
            Offset(x + rng.nextDouble() * 6, y + rng.nextDouble() * 4),
            5.5,
            Paint()
              ..color = palette[rng.nextInt(palette.length)].withValues(
                alpha: 0.85,
              ),
          );
        }
      ...
      for (final ax in [330.0, 660.0, 990.0]) {
        c.drawRect(
          Rect.fromLTWH(ax - 12, t.top, 24, t.height),
          Paint()..color = const Color(0xFF2E3A6E),
        );
      }
      c.drawRect(
        Rect.fromLTRB(t.left, t.bottom, t.right, t.bottom + 30),
        Paint()..color = const Color(0xFF3B4568),
      );
    }
    ...
    c.drawRect(
      Rect.fromLTRB(-40, standBottom - 40, kWorldW + 40, standBottom),
      Paint()
        ..shader = ui.Gradient.linear(
          Offset(0, standBottom - 40),
          Offset(0, standBottom),
          const [Color(0x00000000), Color(0x55000000)],
        ),
    );
  }
  ...
  static void _ground(Canvas c, Stage stage, ViewGeom g, Random rng) {
    ...
    switch (stage) {
        ...
        c.drawRect(
          ...
          Paint()
            ..shader = ui.Gradient.linear(
              ...
              const [Color(0xFF474B57), Color(0xFF2C3039)],
            ),
        );
        for (var i = 0; i < 90; i++) {
          c.drawOval(
            Rect.fromCenter(
              center: Offset(
                rng.nextDouble() * kWorldW,
                top + rng.nextDouble() * (kWorldH - top),
              ),
              width: 80 + rng.nextDouble() * 200,
              height: 14 + rng.nextDouble() * 30,
            ),
            Paint()
              ..color = Color.fromRGBO(
                255,
        ...
        c.drawRect(
          ...
          Paint()
            ..shader = ui.Gradient.linear(
              Offset(0, top),
              const Offset(0, kWorldH),
              const [Color(0xFFF2DCA8), Color(0xFFE2C387)],
            ),
        );
        c.drawRect(
          Rect.fromLTWH(0, top, kWorldW, 70),
          Paint()
            ..shader = ui.Gradient.linear(
              Offset(0, top),
              Offset(0, top + 70),
              const [Color(0xFFD5B67E), Color(0x00D5B67E)],
            ),
        );
        for (var i = 0; i < 700; i++) {
          c.drawCircle(
        ...
        c.drawRect(
          ...
          Paint()
            ..shader = ui.Gradient.linear(
              Offset(0, top),
              const Offset(0, kWorldH),
              const [Color(0xFF2FA352), Color(0xFF1F7A3C)],
            ),
        );
        c.drawRect(
          area,
          Paint()
            ..shader = ui.Gradient.radial(
              Offset(kWorldW / 2, g.groundY + 500),
              1400,
      ...
      case Stage.stadium:
        ...
        while (y < kWorldH) {
          if (i.isEven) {
            c.drawRect(
              Rect.fromLTWH(0, y, kWorldW, band),
              Paint()..color = const Color(0xFF37AD4E),
            );
          }
          y += band;
          band *= 1.12;
        ...
        c.drawRect(
          ...
          Paint()
            ..shader = ui.Gradient.radial(
              Offset(kWorldW / 2, g.groundY + 300),
              1500,
    ...
    c.drawRect(
      Rect.fromLTWH(0, top, kWorldW, 40),
      Paint()
        ..shader = ui.Gradient.linear(
          Offset(0, top),
          Offset(0, top + 40),
  ...
  static void _pitchLines(
    ...
    // Kale sahası (küçük).
    seg(
      Offset(cx - g.goalWidth / 2 - 60, y0),
      Offset(cx - g.goalWidth / 2 - 90, y0 + 230),
    );
    seg(
      Offset(cx + g.goalWidth / 2 + 60, y0),
      Offset(cx + g.goalWidth / 2 + 90, y0 + 230),
  ...
  static void _goal(Canvas c, Stage stage, ViewGeom g) {
    ...
    final netColor = stage == Stage.street
    ...
    for (var d = -1200.0; d < 1200; d += 30) {
      c.drawLine(
        Offset(l + d, top),
        Offset(l + d + (bottom - top), bottom),
        mesh,
      );
      c.drawLine(
        Offset(l + d, bottom),
        Offset(l + d + (bottom - top), top),
        mesh,
      );
    }
    c.restore();
    ...
    // Direkler.
    final postColor = stage == Stage.street
        ? const Color(0xFFD6DAE3)
        : const Color(0xFFFFFFFF);
    final shade = stage == Stage.street
        ? const Color(0xFF8E939F)
        : const Color(0xFFB9C1D3);
    void bar(Rect rect, {bool vertical = true}) {
      final rr = RRect.fromRectAndRadius(rect, const Radius.circular(8));
      c.drawRRect(
        rr.shift(const Offset(6, 6)),
    ...
    bar(
      Rect.fromLTWH(l - post, top - post, r - l + 2 * post, post),
      vertical: false,
    );
    if (stage == Stage.street) {
      // Pas lekeleri.
  ...
  static void _board(Canvas c, Stage stage, ViewGeom g) {
    ...
    c.drawRRect(
      rr,
      Paint()
        ..shader = ui.Gradient.linear(b.topLeft, b.bottomLeft, const [
          Color(0xFF162049),
          Color(0xFF0B1226),
        ]),
    );
    ...
    _ballIcon(
      c,
      Offset(iconX, g.scoreRect.center.dy),
      g.scoreRect.height * 0.42,
    );
    _trophyIcon(
      c,
      Offset(iconX, g.bestRect.center.dy),
      g.bestRect.height * 0.46,
      accent,
    );
  ...
  static void _trophyIcon(Canvas c, Offset o, double r, Color color) {
    ...
    c.drawRRect(
      RRect.fromRectAndRadius(
        Rect.fromCenter(
          center: Offset(o.dx, o.dy + r * 0.85),
          width: r * 1.0,
          height: r * 0.28,
        ),
        const Radius.circular(4),
    ...
    c.drawArc(
      Rect.fromCenter(
        center: Offset(o.dx - r * 0.75, o.dy - r * 0.5),
        width: r * 0.6,
        height: r * 0.8,
      ),
      pi / 2,
  ...
  static void _pauseButton(Canvas c) {
    ...
    for (final dx in [-24.0, 24.0]) {
      c.drawRRect(
        RRect.fromRectAndRadius(
          Rect.fromCenter(center: o.translate(dx, 0), width: 22, height: 80),
          const Radius.circular(8),
        ),
        bar,
      );
